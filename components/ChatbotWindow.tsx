...
const ChatbotWindow: React.FC = () => {
  ...
  useEffect(() => {
    if (!API_KEY) {
      ...
      setMessages([
        {
          role: 'model',
          text: 'Desculpe, meu cérebro está offline agora. A configuração da API está faltando.',
        },
      ]);
      return;
    }
  ...
  return (
    <div
      className={`fixed bottom-0 right-0 md:bottom-6 md:right-6 z-50 transition-all duration-300 ease-in-out ${isChatOpen ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10 pointer-events-none'}`}
    >
      <div className="w-full h-[70vh] md:w-96 md:h-[600px] bg-white/80 backdrop-blur-xl rounded-t-2xl md:rounded-2xl shadow-2xl flex flex-col clay-card">
        ...
        <div className="flex-1 p-4 overflow-y-auto">
          <div className="space-y-4">
            {messages.map((msg, index) => (
              ...
              >
                <div
                  className={`max-w-xs md:max-w-sm px-4 py-2 rounded-2xl ${msg.role === 'user' ? 'bg-blue-500 text-white rounded-br-lg' : 'bg-gray-200 text-gray-800 rounded-bl-lg'}`}
                >
                  {msg.text}
                </div>
              </div>
        ...
        <div className="p-4 border-t border-gray-200">
          <form onSubmit={handleSendMessage} className="flex items-center space-x-2">
            ...
            <button
              type="submit"
              className="p-3 bg-blue-500 text-white rounded-xl clay-button disabled:opacity-50"
